...
@Component({
  ...
  template: `
    <div 
      class="flex h-screen w-screen bg-zinc-950 text-zinc-300 font-sans relative overflow-hidden"
              ...
              @for (project of projectService.projects(); track project.id) {
                 <div 
                   ...
                   class="flex items-center gap-3 p-2 rounded cursor-pointer hover:bg-white/5 transition-colors group justify-center lg:justify-start">
                    <div class="w-2 h-2 rounded-full border border-zinc-600 group-hover:border-white transition-colors shrink-0" [class.bg-white]="isActive(project.id)"></div>
...
export class AppComponent {
  ...
  toggleProject(id: string, event?: MouseEvent) {
    ...
    if (isMultiSelect) {
        ...
        // If clicked project is the ONLY one active, toggle it off.
        // Otherwise (it's inactive OR it's one of many active), make it the single active one.
